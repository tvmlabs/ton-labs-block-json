...
use ton_block::{
    ...
    ConfigParam18Map, ConfigParams, SlashingConfig,
    ConfigProposalSetup,
    ConsensusConfig,
    CurrencyCollection,
...
impl StateParser {
    ...
    pub fn parse_config(&mut self, config: &PathMap) -> Result<()> {
        ...
        if let Err(err) = config.get_obj("p8").map(|p8| {
    ...
    fn parse_state_unchecked(mut self, map: &Map<String, Value>) -> (ShardStateUnsplit, Vec<anyhow::Error>) {
...
pub fn parse_state_unchecked(map: &Map<String, Value>) -> (ShardStateUnsplit, Vec<anyhow::Error>) {
